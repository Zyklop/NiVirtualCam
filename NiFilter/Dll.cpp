	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			if (bRegister)
			{
				...
				rf2.cPins = 1;
				rf2.rgPins = PinsKCam;
				hr = fm->RegisterFilter(CLSID_NiVirtualCam, L"OpenNi Virtual Camera", &pMoniker, &CLSID_VideoInputDeviceCategory,
				                        nullptr, &rf2);
			}
			else
			{
				hr = fm->UnregisterFilter(&CLSID_VideoInputDeviceCategory, nullptr, CLSID_NiVirtualCam);
			}
		}
